 ...
 *
 * Pure constants and categorization logic for ingredients.
...
/**
 * Get all category values as an array (for dropdowns, etc.)
 * @returns {string[]}
 */
...
/**
 * Categorize an ingredient name into a grocery category
 * Uses direct match first, then partial match on keywords
 *
 * @param {string} ingredientName - The name of the ingredient
 * @returns {string} The category value (e.g., "Dairy & Eggs")
 */
 ...
 * @template T
 * @param {T[]} items - Array of items with a category property
 * @returns {Map<string, T[]>} Map of category to items
 */
