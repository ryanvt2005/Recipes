...
const CUISINE_KEYWORDS = {
  ...
  Greek: {
    ...
    ingredients: ['feta', 'kalamata', 'phyllo', 'filo', 'ouzo', 'oregano', 'greek yogurt'],
  },
  Korean: {
    title: [
      'korean',
...
const MEAL_TYPE_KEYWORDS = {
  Breakfast: {
    ...
    description: ['breakfast', 'brunch', 'morning'],
  },
  Lunch: {
    title: [
      'lunch',
    ...
    description: ['lunch', 'midday', 'lunchtime'],
  },
  Dinner: {
    title: [
      'dinner',
    ...
    description: ['dinner', 'supper', 'main course', 'entrée', 'entree', 'weeknight'],
  },
  Dessert: {
    title: [
      'dessert',
    ...
    description: ['dessert', 'sweet treat'],
  },
  Snack: {
    title: [
      'snack',
    ...
    description: ['snack', 'after school', 'game day', 'movie night'],
  },
  Appetizer: {
    title: [
      'appetizer',
    ...
    description: ['appetizer', 'starter', "hors d'oeuvre", 'small plate', 'finger food'],
  },
  'Side Dish': {
    title: [
      'side dish',
    ...
    description: ['side dish', 'side', 'accompaniment'],
  },
  Beverage: {
    title: [
      'beverage',
    ...
    description: ['drink', 'beverage', 'cocktail', 'sip'],
  },
};
...
function detectCuisines(titleLower, ingredientNames, cache) {
  ...
  return sorted.map(([name]) => cache.cuisines.get(name)).filter(Boolean);
}
...
function detectMealTypes(titleLower, descriptionLower, cache) {
  ...
  return sorted.map(([name]) => cache.mealTypes.get(name)).filter(Boolean);
}
...
function detectDietaryLabels(titleLower, descriptionLower, ingredientNames, cache) {
  ...
  return labels.map((name) => cache.dietaryLabels.get(name)).filter(Boolean);
}
...
async function autoTagRecipe(recipe, pool) {
  try {
    ...
    const dietaryLabelIds = detectDietaryLabels(
      titleLower,
      descriptionLower,
      ingredientNames,
      cache
