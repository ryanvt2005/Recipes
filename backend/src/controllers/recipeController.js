...
const {
  ...
  normalizeUrl,
} = require('../services/recipeExtractionService');
...
async function getIngredientsForRecipes(req, res) {
  ...
  try {
    ...
    logger.error('Error fetching ingredients for recipes', {
      error: error.message,
    });
...
async function getRecipes(req, res) {
  ...
  const validSortBy = ['createdAt', 'title', 'updatedAt', 'cookTime', 'totalTime'].includes(sortBy)
    ? sortBy
    : 'createdAt';
...
async function updateRecipe(req, res) {
  ...
  try {
    ...
    if (cuisines) {
      ...
      for (const cuisineId of cuisines) {
        await client.query(`INSERT INTO recipe_cuisines (recipe_id, cuisine_id) VALUES ($1, $2)`, [
          recipeId,
          cuisineId,
        ]);
      }
...
async function getCuisines(req, res) {
  try {
    const result = await pool.query(`SELECT id, name FROM cuisines ORDER BY sort_order, name`);
...
async function getMealTypes(req, res) {
  try {
    const result = await pool.query(`SELECT id, name FROM meal_types ORDER BY sort_order, name`);
